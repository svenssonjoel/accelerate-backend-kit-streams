...
{- BJS:
   ...
   * Plan: Remove all dependency on Accelerate. Instead use some
     other "inlet" (way to create ASTS).
     This is to form a base for experimentation with addition of
     Streaming concepts to the backend-kit AST types in a setting that
     is completely disjoint from Accelerate.

     - Potentially/alternatively, split backend-kit into two parts.
       Independend-backend-kit and Accelerate-backend-kit.
...
module Data.Array.Accelerate.BackendKit.ExperimentalPipeline
       (
...
-- Phase 1 passes:
----------------------------------------
-- This one depends on Accelerate
...
{- BJS, Here I want to skip the initialConversion.
   Instead pass simplified ASTs to the rest of the
   compiler -} 
